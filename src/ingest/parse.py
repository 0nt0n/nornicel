...
def _convert_doc_to_docx(path: str) -> str:
    """
    Конвертирует .doc -> .docx через LibreOffice.
    Возвращает путь к временному .docx.
    """
    ...
    converted = os.path.join(tmpdir, name)

    if not os.path.exists(converted):
        raise RuntimeError(f"Не удалось конвертировать {path}")
